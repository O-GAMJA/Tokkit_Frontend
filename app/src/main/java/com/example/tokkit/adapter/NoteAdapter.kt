        ...
        when (holder) {
            is CardViewHolder -> {
                with(holder.binding) {
                    ...
                    // previewContent가 있으면 사용하고, 없으면 content 사용
                    val noteContent = if (!note.previewContent.isNullOrEmpty()) {
                        note.previewContent
                    } else if (!note.content.isNullOrEmpty()) {
                        note.content.take(100).plus("...")
                    } else {
                        "내용 없음"
                    }

                    markwon.setMarkdown(tvContent, noteContent)
